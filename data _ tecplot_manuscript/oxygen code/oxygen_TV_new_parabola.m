...
j = linspace(low_lim_x,up_lim_x,69);

for m = 1:1:69
% h_theta xi calculation 
for i = 1:1:numRows
h_theta_1 = @(xi) (x(i)+1).^2.*exp(-(x(i)-xi).^2/gamma);
h_theta1 = integral(h_theta_1,low_lim_x,up_lim_x);
h_theta_2 = @(xi) exp(-(x(i)-xi).^2/gamma);
...
h_theta_1j = @(xi) (x(i)+1).^2.*exp(-(j(m)-xi).^2/gamma);
h_theta1j = integral(h_theta_1j,low_lim_x,up_lim_x);
h_theta_2j = @(xi) exp(-(j(m)-xi).^2/gamma);
