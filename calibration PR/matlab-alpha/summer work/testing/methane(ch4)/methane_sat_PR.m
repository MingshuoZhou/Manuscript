...
hold on
plot(sqrt(Tr_list),sqrt(alpha_PR),'h','MarkerSize',8,'MarkerFaceColor','r','LineWidth',1.2)
plot(sqrt(Tr_list),sqrt(alpha_SRK),'^','MarkerSize',8,'MarkerFaceColor','g','LineWidth',1.2)
plot(sqrt(Tr_list),sqrt(alpha_soave_PR),'r','LineWidth',2)
plot(sqrt(Tr_list),sqrt(alpha_soave_SRK),'g','LineWidth',2)
plot(sqrt(Tr_list),sqrt(alpha_Gasem),'b','LineWidth',2)
% plot(Tr_list,alpha_twu,'g','LineWidth',2)
hold off
legend('PR NIST','SRK NIST','Soave PR','Soave SRK','Gasem','Twu')
xlabel('Tr^{0.5}')
ylabel('Alpha^{0.5}')
title('EoS Alpha functions value comparison for methane')