clear 
clc
load nitrogen_rho_different_pressure_data.mat
%PR Nitrogen
...
for i = 1:1:34
   ...
   alpha_PR_01(i) = ((R*T)/(Vm(i,1)-b)-P(1))*((Vm(i,1)^2+b*Vm(i,1))/a);
   alpha_PR_03(i) = ((R*T)/(Vm(i,2)-b)-P(2))*((Vm(i,2)^2+b*Vm(i,2))/a);
   alpha_PR_05(i) = ((R*T)/(Vm(i,3)-b)-P(3))*((Vm(i,3)^2+b*Vm(i,3))/a);
   alpha_PR_1(i) = ((R*T)/(Vm(i,4)-b)-P(4))*((Vm(i,4)^2+b*Vm(i,4))/a);
   alpha_PR_3(i) = ((R*T)/(Vm(i,5)-b)-P(5))*((Vm(i,5)^2+b*Vm(i,5))/a);
   alpha_PR_5(i) = ((R*T)/(Vm(i,6)-b)-P(6))*((Vm(i,6)^2+b*Vm(i,6))/a);
   alpha_PR_10(i) = ((R*T)/(Vm(i,7)-b)-P(7))*((Vm(i,7)^2+b*Vm(i,7))/a);
   alpha_PR_20(i) = ((R*T)/(Vm(i,8)-b)-P(8))*((Vm(i,8)^2+b*Vm(i,8))/a);
...
xlabel('Tr')
ylabel('Alpha')
title('SRK EoS Alpha functions value comparison for Nitrogen')