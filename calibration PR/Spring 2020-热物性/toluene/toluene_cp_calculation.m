...
clear all
clc
load toluene_data.mat
omega = 0.263; %Acentric factor
Tc = 591.8; %K
Pc = 4100000; %Pa
R = 8.31446; %J/(mol*K)
...
for i = 1:1:101
    ...
    Cp_0 = -310.29+5.640685*T-2.81410224E-3*T^2+2.4913887E-7*T^3;
    
    Cp_0_list(i) = Cp_0/1000; %kJ/(kg*k)
    ...
    e_0 = -310.29*T+5.640685/2*T^2-(2.81410224E-3)/3*T^3+(2.4913887E-7)/4*T^4-R/M*T-(-310.29*Tr+5.640685/2*Tr^2-(2.81410224E-3)/3*Tr^3+(2.4913887E-7)/4*Tr^4-R/M*Tr);
...
title('SRK EoS Constant pressure specific heat for toluene')
axis([300 800 1.5 6]);
hold off
...
title('SRK Specific Enthalpy for toluene')
% axis([60 400 -200 400]);
hold off
