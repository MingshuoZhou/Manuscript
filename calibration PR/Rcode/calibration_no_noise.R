...
# read data
data.df <- read.csv("c12h26.csv")
X <- as.matrix(data.df[,1:2])
Z <- data.df[,3]

# estimate parameters
out <- optim(c(0.5, 0.5,3000000, 3000000, 3000000), nl, method="L-BFGS-B", lower=c(1e-6,1e-6,-5,-5,-5), upper=c(1-1e-6, 1-1e-6,3000000,3000000,3000000), X=X, Y=Z) 

# prediction
newdata.df <- read.csv("c12h26.csv")
